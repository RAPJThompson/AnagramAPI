...
class TestCases < Test::Unit::TestCase
  ...
  def test_fetching_anagrams
    # fetch words
    res = @client.get('/words/read.json')
    ...
    assert_not_nil(body['words'])

    expected_anagrams = %w(dare dear)
    assert_equal(expected_anagrams, body['words'].sort)
  end

  def test_fetching_anagrams_with_limit
    # fetch words with limit
    res = @client.get('/words/read.json', 'limit=1')

    assert_equal('200', res.code, "Unexpected response code")

    body = JSON.parse(res.body)

    assert_equal(1, body['words'].size)
  end

  def test_fetch_for_word_with_no_anagrams
    # fetch words with limit
    res = @client.get('/words/zyxwv.json')

    assert_equal('200', res.code, "Unexpected response code")

    body = JSON.parse(res.body)

    assert_equal(0, body['words'].size)
  end
